  ...
  void _mostrarDialogoEmpleado([Empleado? empleado]) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title:
            Text(empleado == null ? '➕ Nuevo Empleado' : '✏️ Editar Empleado'),
        content: Form(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Form(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Empleado: ${_empleadoSeleccionado!.nombre}',
              ...
              DropdownButtonFormField<String>(
                ...
                items: ['Salario', 'Bono', 'Extra', 'Comisión', 'Otro']
            ...
            onPressed: () {
              if (_formPagoKey.currentState!.validate()) {
                final pago = PagoEmpleado(
                  ...
                  concepto: _conceptoController.text.isNotEmpty
                      ? _conceptoController.text
                      : 'Pago General',
                  fecha: DateTime.now(), // Agregado el campo requerido
                );
  ...
  Widget build(BuildContext context) {
    return Consumer<AppProvider>(
      builder: (context, provider, child) {
        ...
        final pagos = provider.pagosEmpleados;

        return Scaffold(
          appBar: AppBar(
            title: const Text('👥 Cobros a Empleados',
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            subtitle: Text(
                                '${e.cargo} - \$${e.salarioBase.toStringAsFixed(2)}'),
                            trailing: Row(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                IconButton(
                                    ...
                                    onPressed: () =>
                                        _mostrarDialogoEmpleado(e)),
                                IconButton(
                                    icon: const Icon(Icons.delete,
                                        size: 20, color: Colors.red),
                                    onPressed: () {
                                      provider.eliminarEmpleado(e.id);
                                      if (_empleadoSeleccionado?.id == e.id)
                                        setState(
                        ...
                        children: [
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(_empleadoSeleccionado!.nombre,
                                    style: const TextStyle(
                                        fontSize: 20,
                                        fontWeight: FontWeight.bold)),
                                Text('Cargo: ${_empleadoSeleccionado!.cargo}'),
                                Text(
                                    'Salario base: \$${_empleadoSeleccionado!.salarioBase.toStringAsFixed(2)}'),
                                const SizedBox(height: 12),
                                ElevatedButton.icon(
                          ...
                          Expanded(
                            child: ListView.builder(
                              itemCount: pagos
                                  .where((p) =>
                                      p.empleadoId == _empleadoSeleccionado!.id)
                                  .length,
                              itemBuilder: (context, index) {
                                final pago = pagos
                                    .where((p) =>
                                        p.empleadoId ==
                                        _empleadoSeleccionado!.id)
                                    .toList()[index];
                                return Card(
                                  ...
                                  child: ListTile(
                                    ...
                                    subtitle: Text(
                                        '${pago.concepto} - ${pago.fecha.day}/${pago.fecha.month}/${pago.fecha.year}'),
                                    trailing: pago.concepto != null
